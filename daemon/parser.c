...
static struct snapraid_device* find_device(struct snapraid_state* state, int number, char* name, const char* file)
{
	int index;
...
static void process_content_data(struct snapraid_state* state, char** map, size_t mac)
{
	...
	struct snapraid_disk* data = find_disk(&state->array.disk_list, task->number, name, DISK_DATA);
	...
	if (strcmp(tag, "serial") == 0)
		...
		if (device->power != power) {
			...
			if (power == POWER_ACTIVE) {
				struct snapraid_disk* disk_spunup = find_disk(&state->array.disk_list, task->number, disk, DISK_UNDEFINED);
				if (disk_spunup != 0) {
...
static void process_error_io(struct snapraid_state* state, char** map, size_t mac)
{
	...
	struct snapraid_disk* disk = find_disk(&state->array.disk_list, task->number, map[2], DISK_DATA);
...
static void process_parity_error_io(struct snapraid_state* state, char** map, size_t mac)
{
	...
	struct snapraid_disk* disk = find_disk(&state->array.disk_list, task->number, map[2], DISK_PARITY);
	pulse(state, PULSE_DISKS | PULSE_ACTIVITY);
	++disk->error_io;
	++task->error_io;
}
