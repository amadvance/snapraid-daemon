 ...
 */
void parser_mapping_clear(struct snapraid_state* state)
{
...
static int process_line(struct snapraid_state* state, char** map, size_t mac)
{
	...
	if (!tommy_list_empty(&state->parser_association)) {
		parser_mapping_process(state);
		parser_mapping_clear(state); /* clear to avoid to enter here again */
	}
