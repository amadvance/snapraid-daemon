...
static void parser_device_remove(struct snapraid_state* state, struct snapraid_device* device)
{
	...
	for (tommy_node* d = tommy_list_head(&state->array.disk_list); d != 0; d = d->next) {
		...
		for (tommy_node* p = tommy_list_head(&disk->device_pointer_list); p != 0; ) {
			struct snapraid_device_pointer* pointer = p->data;
			tommy_node* p_next = p->next;
	...
	tommy_list_remove_existing(&state->device_catalog, &device->node);
	device_free(device);
}
	...
	for (tommy_node* i = tommy_list_head(&state->device_catalog); i != 0; ) {
		...
		if (contaminated) {
			...
			if (conflict) {
				...
				pulse(state, PULSE_DISKS);
				sncpy(device->file, sizeof(device->file), "disconnected");
			}
		}
...
static void remove_unreferenced_disks(struct snapraid_state* state, struct snapraid_task* task)
{
	...
	for (tommy_node* i = tommy_list_head(&state->array.disk_list); i != 0; ) {
...
static void clear_disk_accumulator(struct snapraid_state* state)
{
	for (tommy_node* i = tommy_list_head(&state->array.disk_list); i; i = i->next) {
		...
		if (disk->transient_error_io != 0 || disk->transient_error_data != 0) {
			pulse(state, PULSE_DISKS);
			disk->transient_error_io = 0;
			disk->transient_error_data = 0;
		}
	}
}
...
static struct snapraid_device* find_device(struct snapraid_state* state, const char* file)
{
	struct snapraid_device* device = device_catalog_find_by_file(state, file);
	if (device)
		return device;
	...
	tommy_list_init(&device->temp_list);
	tommy_list_insert_tail(&state->device_catalog, &device->node, device);
...
static struct snapraid_device_pointer* find_device_pointer(struct snapraid_disk* disk, int number, struct snapraid_device* device, int split_index)
{
	for (tommy_node* i = tommy_list_head(&disk->device_pointer_list); i != 0; i = i->next) {
		...
		if (pointer->split_index == split_index && pointer->device == device) {
...
static void process_content_info(struct snapraid_state* state, char** map, size_t mac)
{
	...
	if (strcmp(tag, "file") == 0) {
		...
		pulse_stru64(state, PULSE_ARRAY, &state->array.block_unscrubbed, val);
	} else if (strcmp(tag, "block_unsynced") == 0) {
		pulse_stru64(state, PULSE_ARRAY, &state->array.block_unsynced, val);
	} else if (strcmp(tag, "block") == 0) {
		pulse_stru64(state, PULSE_ARRAY, &state->array.block_total, val);
	} else if (strcmp(tag, "probe_unixtime") == 0) {
		stri64(&state->array.content_probe_unixtime, val);
	} else if (strcmp(tag, "read_unixtime") == 0) {
		stri64(&state->array.content_last_unixtime, val);
...
static void process_info(struct snapraid_state* state, char** map, size_t mac)
{
	...
	find_disk_device(state, task->number, disk, file);
}
...
static void process_attr(struct snapraid_state* state, char** map, size_t mac)
{
	...
	struct snapraid_device* device = find_disk_device(state, task->number, disk, file);
	if (!device)
		return;
...
static void process_bucket(struct snapraid_state* state, char** map, size_t mac)
{
	...
	if (strcmp(tag, "entry") == 0) {
		...
		if (stru64(&time_at, val) == 0
			...
			tommy_list_insert_tail(&state->array.bucket_parse_list, &bucket->node, bucket);
		}
...
static void process_list(struct snapraid_state* state, char** map, size_t mac)
{
	...
	if (strcmp(tag, "scan_begin") == 0) {
		...
		/* do not pulse because this is temporary storage for parsing */
		bucket_cleanup(&state->array.bucket_parse_list);
		...
		bucket_move(&state->array.bucket_parse_list, &state->array.bucket_list);
	}
}
...
static void process_conf(struct snapraid_state* state, char** map, size_t mac)
{
	...
	if (strcmp(map[1], "file") == 0) {
		pulse_str(state, PULSE_ARRAY, state->array.engine_conf, sizeof(state->array.engine_conf), map[2]);
	}
}
...
static void process_content(struct snapraid_state* state, char** map, size_t mac)
{
	...
	pulse_str(state, PULSE_ARRAY, state->array.content, sizeof(state->array.content), map[1]);
}
...
static void process_version(struct snapraid_state* state, char** map, size_t mac)
{
	...
	/* full version text */
	pulse_str(state, PULSE_ARRAY, state->engine_version, sizeof(state->engine_version), s);
	...
	state->parser_version_major = major;
	state->parser_version_minor = minor;
}
...
static void process_blocksize(struct snapraid_state* state, char** map, size_t mac)
{
	...
	pulse_struint(state, PULSE_ARRAY, &state->array.blocksize, map[1]);
}
...
static void process_unixtime(struct snapraid_state* state, char** map, size_t mac)
{
	...
	stri64(&state->array.last_time, map[1]);
	 ...
	 */
	temperature_cleanup_devices(state, state->array.last_time);
}
...
static void process_command(struct snapraid_state* state, char** map, size_t mac)
{
	...
	pulse_str(state, PULSE_ARRAY, state->array.last_cmd, sizeof(state->array.last_cmd), val);
...
static void process_summary(struct snapraid_state* state, char** map, size_t mac)
{
	...
	if (strcmp(tag, "equal") == 0)
		stri64(&state->array.diff_parse.diff_equal, val);
	else if (strcmp(tag, "added") == 0)
		stri64(&state->array.diff_parse.diff_added, val);
	else if (strcmp(tag, "removed") == 0)
		stri64(&state->array.diff_parse.diff_removed, val);
	else if (strcmp(tag, "updated") == 0)
		stri64(&state->array.diff_parse.diff_updated, val);
	else if (strcmp(tag, "moved") == 0)
		stri64(&state->array.diff_parse.diff_moved, val);
	else if (strcmp(tag, "copied") == 0)
		stri64(&state->array.diff_parse.diff_copied, val);
	else if (strcmp(tag, "relocated") == 0)
		stri64(&state->array.diff_parse.diff_relocated, val);
	else if (strcmp(tag, "restored") == 0)
		stri64(&state->array.diff_parse.diff_restored, val);
	...
	if (strcmp(tag, "exit") == 0) {
		...
		case CMD_SCRUB :
			pulse(state, PULSE_ARRAY);
			state->array.scrub_time = task->unix_start_time;
			break;
		case CMD_DIFF :
			pulse(state, PULSE_ARRAY);
			state->array.diff_time = task->unix_start_time;
 ...
 * A tag must start with a lowercase letter, contain only lowercase
 * letters, digits, or underscores, and end with a colon.
 */
static int is_tag_format(const char* s)
{
	...
	/* tag name must begin with a lowercase letter */
	if (!islower((unsigned char)*p))
		return 0;

	/* tag name consists of lowercase letters, digits, and underscores */
	while (*p != 0 && *p != ':') {
...
int parse_past_log(struct snapraid_state* state)
{
	...
	/* retain no more than HISTORY_PAST_DAYS of tasks in the history */
