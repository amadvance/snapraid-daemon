...
static const int METRICS_HEALTH_CODES[] = { HEALTH_PASSED, HEALTH_CORRUPT, HEALTH_PREFAIL, HEALTH_FAILING, HEALTH_PENDING };
static const int METRICS_HEALTH_NUMERIC[] = { 1, 2, 3, 4, 0 };
#define METRICS_HEALTH_COUNT 5

static const struct { int kind; const char* role; } METRICS_DISK_ROLES[] = {
	{ DISK_DATA,   "data" },
	{ DISK_PARITY, "parity" },
	{ DISK_EXTRA,  "extra" },
};
...
static int handler_metrics(struct mg_connection* conn, void* cbdata)
{
	...
	int array_health_num = 0;
	for (int i = 0; i < METRICS_HEALTH_COUNT; ++i) {
		if (global->health == METRICS_HEALTH_CODES[i]) {
			array_health_num = METRICS_HEALTH_NUMERIC[i];
			break;
		}
	}
	...
	for (int r = 0; r < METRICS_DISK_ROLE_COUNT; ++r) {
		for (tommy_node* i = tommy_list_head(&state->disk_list); i; i = i->next) {
			...
			if (disk->kind != METRICS_DISK_ROLES[r].kind)
				continue;
			int dh = health_disk(disk, 0, 0);
			int disk_health_num = 0;
			for (int h = 0; h < METRICS_HEALTH_COUNT; ++h) {
				if (dh == METRICS_HEALTH_CODES[h]) {
					disk_health_num = METRICS_HEALTH_NUMERIC[h];
					break;
				}
			}
			ss_prints(&s, "snapraid_disk_health{disk=\"");
			ss_prints_prometheus_escaped(&s, disk->name);
			ss_printf(&s, "\",role=\"%s\"} %d\n", METRICS_DISK_ROLES[r].role, disk_health_num);
		}
	}
	ss_prints(&s, "\n");
