...
Getting Started
	...
	Upon installation, the daemon provides a default configuration
	`/etc/snapraidd.conf` that enables the REST API and the Web UI. By
	default, these services are accessible via a web browser at the
	following address:
	...
	of SnapRAID commands. Consequently, on the very first run, this
...
Configuration
	...
	real-world environment, refer to the `snapraidd.conf.example` file
  ...
  Logging & Notifications
    ...
    notify_result
	Specifies a command triggered after every high-level task (maintenance,
